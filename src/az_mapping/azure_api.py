...
_spot_cache: dict[str, tuple[float, dict[str, str]]] = {}
...
def _fetch_spot_batch(
    ...
    Returns a dict mapping VM size → score (High / Medium / Low).
...
def get_spot_placement_scores(
    ...
    Returns ``{"scores": {vmSize: score}, "errors": [...]}``.
    ...
    for i, batch in enumerate(batches):
        try:
            ...
            merged_scores.update(batch_scores)
        except Exception as exc:
