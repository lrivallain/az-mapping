...
- **Fluent v3 wave C — chat & About surfaces (#165)** – Second-wave migration replaces every remaining hand-rolled pill / badge / spinner / tooltip / chip with a Fluent web component:
  - **Chat thinking dots** → `<fluent-spinner size="tiny">`.
  - **Tool-call status badges** → `<fluent-badge color="warning|success">` with an embedded `<fluent-spinner size="tiny">` during the call. The `.calling`/`.done` class hooks are kept so the existing click-to-open-details handler still finds them.
  - **Choice chips** → `<fluent-button class="chat-choice-chip" appearance="outline" shape="circular" size="small">`. The "used" state is now driven by the native `disabled` attribute.
  - **Chat-mode toggle (pill switch)** → `<fluent-tablist appearance="subtle">` + `<fluent-tab data-mode="…">`. The streaming guard now also reverts the tablist's visual selection so a click during a streaming answer no longer leaves a stale tab selected.
  - **Chat header buttons** → `<fluent-button icon-only>` + sibling `<fluent-tooltip anchor="…">` (the CSS-only `[data-tooltip]:hover::after` pseudo-element is gone).
  - **About modal** — version pill → `<fluent-badge appearance="tint" shape="circular">`; Homepage / GitHub / API docs links → `<fluent-anchor-button shape="circular">`; plugin row icons → `<fluent-avatar color="brand">`; plugin version chips → `<fluent-badge>`.
  - **Loading spinners** in the Plugin Manager (global status bar + manual-install validation), the catalog index, the topology tab, the planner tab, and the spot-modal score-fetcher all become `<fluent-spinner>`.
  - **Plugin Manager source badges** (`pypi`, `github`, `built-in`, `not loaded`) → `<fluent-badge appearance="filled" color="…">`.

### Removed

- **~150 lines of hand-rolled CSS** superseded by Fluent web components (wave C, #165): the `[data-tooltip]:hover::after` pseudo-tooltip system, `.chat-thinking` dots + `@keyframes chat-bounce`, `.chat-tool-badge` colour variants + dark-mode overrides, the legacy `.spin` / `@keyframes spin` helper, `.chat-choice-chip` styling, the `.chat-mode-toggle` pill switch (button-by-button), `.about-version`, `.about-links .btn` rounding override, `.about-plugin-icon`, and `.about-plugin-version`. Layout-only rules (`.chat-suggestions`, `.chat-choices-list`, `.about-links`, `.about-plugin-row`, `.about-plugin-name`, `.about-plugin-link`) and the click affordance on `.chat-tool-badge.done` are preserved.
...
- **`frontend.instructions.md`** – Refreshed CSS guidance to point at `DESIGN.md` as the single source of truth and added a “Component library (Fluent UI Web Components v3)” section.- **Fluent CDN loader (#165)** – `templates/index.html` and `templates/login.html` now load `@fluentui/web-components@beta` as a self-registering **UMD** bundle from jsDelivr (`dist/web-components.min.js`); `setTheme` and the theme tokens are imported separately via `+esm`. The previous `<script type="module" src="https://unpkg.com/@fluentui/web-components@beta">` form was rejected by the browser because the raw ESM build uses a bare `import "tslib"` that the module resolver cannot satisfy and because unpkg's `@beta` tag redirects to the UMD entry point. CSP is unchanged — jsDelivr is already on the allow-list.
- **DESIGN.md naming glossary (#165)** – Added a "Fluent 2 vs Fluent UI v3" glossary clarifying that Fluent 2 is the design system (tokens, type, spacing, elevation) and Fluent UI Web Components v3 is the library that implements it. Standardised phrasing across `DESIGN.md`, `frontend.instructions.md`, and this changelog.
- **Bootstrap → core compatibility runtime (#165)** – The Bootstrap 5.3 CSS + JS bundles are now framed as a **plugin compatibility runtime** rather than a core layout dependency. The migration plan in `DESIGN.md` marks `.modal`, `.offcanvas`, and `#mainTabs` as "kept" because they are part of the plugin contract (sibling plugins emit Bootstrap markup and call `bootstrap.Modal.getOrCreateInstance()`). Core templates have started migrating their own layout away from Bootstrap utilities.
- **Core templates use `app-*` layout namespace (#165)** – `templates/index.html` and `templates/login.html` no longer use Bootstrap layout utilities (`.navbar`, `.container-fluid`, `.card`, `.alert`, `.btn`, footer/grid utilities) for their own chrome. A new `app-*` CSS namespace in `style.css` (built on `--fl-*` tokens) materialises the body, navbar, main container, selector bar, plugin tab-pane wrappers, footer, and inline alert callouts. The About-modal Close button and the login screen's specific-tenant submit are now `<fluent-button>`s; login alert callouts are now `app-alert` blocks. Bootstrap CSS + JS are still loaded for plugin compatibility, so `.modal`, `.offcanvas`, `.nav-tabs`, `.input-group`/`.form-*`, `.bi-*` icons, and small utility classes (`.d-flex`, `.gap-*`, `.d-none`, `.text-*`, `.me-*`, …) continue to work everywhere — plugins are unaffected.
- **Built-in plugins migrated to Fluent v3 (#165)** – `internal_plugins/topology` and `internal_plugins/planner` no longer emit Bootstrap `.btn` markup for their own actions. The Topology tab's *Select visible / Clear all / Load / Export PNG / Export CSV* controls and the Planner's *Load SKUs / Export CSV / Spot toggle / Prices toggle* controls are now `<fluent-button>` and `<fluent-switch>` web components. The Spot Score modal's *Cancel / Get Score* footer buttons are `<fluent-button>` (the surrounding Bootstrap `.modal` host is kept because both plugins drive it via `bootstrap.Modal`). Error banners in both tabs now use the `app-alert` namespace instead of `.alert .alert-danger`. The shared `sku-detail-modal.js` and the surrounding `.card`/`.modal` Bootstrap chrome are intentionally **unchanged** so the plugin contract observed by sibling plugins stays identical.
### Fixed
